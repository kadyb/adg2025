...
## Czym jest algorytm? {.center style="text-align: center;"}
...
**Złożoność algorytmu** to miara wzrostu zużycia zasobów przez algorytm wraz
ze wzrostem wielkości danych wejściowych.
...
::: {.fragment}
Do opisania złożoności algorytmu powszechnie stosowana jest **notacja dużego
$O$**, opisująca najgorszy scenariusz działania.
:::
...
f_const = rep(1, length(n)) # O(1)
f_log = log2(n)             # O(log n)
f_linear = n                # O(n)
f_nlogn = n * log2(n)       # O(n log n)
f_quad = n^2                # O(n^2)
```

```{r}
par(mar = c(8, 4, 4, 2))
plot(NULL, type = "n", xlim = c(1, 100), ylim = c(0, 100),
     xlab = "Rozmiar danych wejściowych (n)", ylab = "f(n)",
     main = "Porównanie złożoności algorytmów")
lines(n, f_const, col = "black", lwd = 2)
lines(n, f_log, col = "blue", lwd = 2)
lines(n, f_linear, col = "green", lwd = 2)
lines(n, f_nlogn, col = "purple", lwd = 2)
lines(n, f_quad, col = "orange", lwd = 2)
legend("bottom", lwd = 2, inset = c(0, -0.5), xpd = TRUE, horiz = TRUE,
       legend = c("O(1)", "O(log n)", "O(n)", "O(n log n)", "O(n^2)"),
       col = c("black", "blue", "green", "purple", "orange"), bty = "n")
```
...
::: {.column width="30%"}
![](https://julialang.org/assets/infra/logo.svg){height=130}
...
::: {.column width="20%"}
![](https://raw.githubusercontent.com/Benio101/cpp-logo/refs/heads/master/cpp_logo.svg){height=130}
...
::: {.column width="33%"}
![[QGIS](https://www.qgis.org/)](https://qgis.org/img/logosign.svg){height=250}
:::

::: {.column width="33%"}
![[Saga GIS](https://saga-gis.sourceforge.io/en/index.html)](https://saga-gis.sourceforge.io/_images/logo_saga_big.png){height=250}
:::

::: {.column width="33%"}
![[Grass GIS](https://grass.osgeo.org/)](https://grass.osgeo.org/images/logos/grass-green-no-text.svg){height=250}
:::
...
::: {style="text-align: center;"}
[**DOKUMENTACJA**](https://docs.qgis.org/3.40/en/docs/user_manual/processing_algs/qgis/)
:::
...
::: {style="font-size: 40%;"}
<https://saga-gis.sourceforge.io/saga_tool_doc/9.3.1/ta_morphometry_0.html>
:::
:::
...
::: {style="font-size: 80%;"}
Wykorzystanie [QGIS CLI](https://docs.qgis.org/3.40/en/docs/user_manual/processing/standalone.html)
...
::::: columns
::: {.column width="20%"}
![](https://numfocus.org/wp-content/uploads/2021/06/GDALLogoColor300x300-1.png)
:::

::: {.column width="80%"}
::: {style="font-size: 70%;"}
::: {.nonincremental}
Od wersji 3.11 dostępne są algorytmy geoprzestrzenne umożliwiające przetwarzanie
danych rastrowych (47 algorytmów) oraz wektorowych (29 algorytmów).
Przykładowo:
...
::: {style="text-align: center;"}
[**DOKUMENTACJA**](https://gdal.org/en/stable/programs/index.html#gdal-application)
:::
