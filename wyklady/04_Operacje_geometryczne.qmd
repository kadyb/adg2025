...
```{r}
# minimal bounding rotated rectangle
plot(pts[50:65], xlim = c(0, 100), ylim = c(0, 100))
r_bbox = hull(pts[50:65], type = "rectangle")
...
Lokalizacja centroidów i punktów na powierzchni nie jest identyczna.
...
plot(poly, lty = 2, xlim = c(40, 60), ylim = c(30, 40),
     main = "Δx = 0.5  Δy = 0.5")
plot(poly2, col = "lightblue", add = TRUE)
plot(cent, col = "red", add = TRUE, cex = 1.4)
```
...
plot(poly2, col = "lightblue", add = TRUE)
plot(vect("POINT (50 40)"), col = "red", add = TRUE, cex = 1.4)
```
...
- Reprojekcje.
- ... i wiele innych (patrz m. in. dokumentacja [QGIS](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectorgeometry.html)).
