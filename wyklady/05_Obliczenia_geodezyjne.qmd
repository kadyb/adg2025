...
![](https://www.nasa.gov/wp-content/uploads/2023/03/618486main_earth_full.jpg)
...
![](https://www.esa.int/images/geoid_bumpy_RGB_L,2.jpg){fig-align="center"}
...
```{r}
file = "_images/Ellipsoide.svg"
if (!file.exists(file)) {
  url = "https://upload.wikimedia.org/wikipedia/commons/3/33/Ellipsoide.svg"
...
tworząc płaską mapę.
- Stożkowe -- Ziemia rzutowana jest na stożek, który również jest rozwijany.
...
::: {style="text-align: center;"}
[**Więcej przykładów**](https://proj.org/en/latest/operations/projections/all_images.html)
:::
...
![](https://map-projections.net/img/figs/mercator-tissot-big.jpg){fig-align="center"}

::: {style="text-align: center;"}
::: {style="font-size: 60%;"}
Tobias Jung ([Compare Map Projections](https://map-projections.net))
:::
:::
...
- Każde odwzorowanie powoduje pewne zniekształcenia.
- Różne cele wymagają odmiennych podejść.
- Odwzorowanie jest zawsze przybliżeniem rzeczywistości.
...
**Nie ma jednoznacznej reguły dotyczącej kolejności zapisu szerokości i długości
...
```{r}
file = "_images/Euclidean_distance_2d.svg"
if (!file.exists(file)) {
  url = "https://upload.wikimedia.org/wikipedia/commons/5/55/Euclidean_distance_2d.svg"
  download.file(url, file, mode = "wb")
}
```
...
```{r}
file = "_images/Ortodroma.svg"
if (!file.exists(file)) {
  url = "https://upload.wikimedia.org/wikipedia/commons/c/c2/Ortodroma.svg"
  download.file(url, file, mode = "wb")
}
```
...
punktami na powierzchni sfery. Istnieje kilka sposobów na jej obliczenie,
jednak dwa najistotniejsze to:
...
$\Delta\lambda = \lambda_2 - \lambda_1$ (różnica w odległości geograficznej),

$\Delta\phi = \phi_2 - \phi_1$ (różnica w szerokości geograficznej),
...
2. Oblicz skumulowaną odległość euklidesową w trzech wymiarach (X, Y, Z)
...
```{r}
plot(elev / 1000 ~ dist, type = "l", ylim = c(0, 2), col = "brown", lwd = 3,
     xlab = "Odległość planarna [km]", ylab = "Wysokość [km n.p.m]")
abline(v = c(0, 280), lty = 2)
```

Odległość planarna: 280 km

[Odległość powierzchniowa:]{style="color:brown;"} 283 km

## Powierzchnia poligonów

**Który z przedstawionych obszarów jest większy?**
